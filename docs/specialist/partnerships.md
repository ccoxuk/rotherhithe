...
```mermaid
flowchart TD
    Start([New Project]) --> Stage1
    
    subgraph Stage1["1️⃣ DISCOVERY & MAPPING"]
        S1A[Identify Potential Partners]
        S1B[Assess Mutual Value]
        S1C[Prioritise by Impact]
        S1A --> S1B --> S1C
    end
    
    Stage1 --> Stage2
    
    subgraph Stage2["2️⃣ APPROACH & NEGOTIATE"]
        S2A[Initial Contact/<br/>Pitch Benefits]
        S2B[Clarify Expectations<br/>& Deliverables]
        S2C[Agree Terms<br/>& Timeline]
        S2A --> S2B --> S2C
    end
    
    Stage2 --> Stage3
    
    subgraph Stage3["3️⃣ FORMALISE"]
        S3A[Draft MoU/SLA]
        S3B[Confirm Insurance<br/>& Safeguarding]
        S3C[Sign Agreement]
        S3A --> S3B --> S3C
    end
    ...
    subgraph Stage4["4️⃣ ACTIVATE"]
        S4A[Integrate into<br/>Operations]
        S4B[Maintain Regular<br/>Communication]
        S4C[Deliver Mutual<br/>Benefits]
        S4A --> S4B --> S4C
    end
    
    Stage4 --> Stage5
    
    subgraph Stage5["5️⃣ EVALUATE"]
        S5A[Measure Value<br/>Delivered]
        S5B[Gather Partner<br/>Feedback]
        S5C[Assess Relationship<br/>Health]
        S5A --> S5B --> S5C
    end
    
    Stage5 --> Stage6
    
    subgraph Stage6["6️⃣ SUSTAIN OR CONCLUDE"]
        S6A{Continue<br/>Partnership?}
        S6B[Renew/Expand<br/>Agreement]
        S6C[Close Gracefully<br/>& Thank]
        S6D[Document Lessons<br/>Learned]
        S6A -->|Yes| S6B
        S6A -->|No| S6C
        S6B --> S6D
        S6C --> S6D
    ...
    S6D --> NextProject
    S6D -.->|Apply Learnings| Stage1
    
    NextProject([Future Projects<br/>Benefit from<br/>Partnership Experience])
